"""Select the refusal direction (layer and token position) that best bypasses refusal.

Stage 2 of the refusal-direction pipeline. Sweeps every (layer, position) mean
difference between harmful and harmless activations, scoring each on bypass,
induce, and KL metrics against a held-out validation set. Picks the direction
with the lowest bypass score under the KL constraint (falling back to a relaxed
threshold) and saves it to optimal_direction.pt under activations/<model>/.
Select the model with the MODEL_ID env var; Gemma-3 multimodal models are
...
def embed_module(model):
    """Return the token-embedding submodule, accounting for multimodal nesting."""
...
def load_training_prompts():
    """Return 128 harmful (AdvBench) and 128 harmless (Alpaca) training prompts."""
    random.seed(SEED)
...
def step_1_discover_tokens(model, tokenizer, harmful_train_prompts):
    """Return the refusal-token ids covering 90% of greedy first tokens, cached to disk."""
...
def step_2_prepare_validation(model, tokenizer, refusal_ids, train_h, train_s):
    """Build and cache tokenized held-out harmful and harmless validation sets.

    Draws prompts disjoint from the training split, keeps up to 32 of each whose
    refusal log-odds match their label, and saves the tokenized batches to
    VAL_DATA_FILE.
    """
...
def compute_batched_metric(model, input_ids, attention_mask, layer, vector, refusal_tensor, mode, clean_probs=None, num_layers=32):
    """Return the mean metric over batches for one intervention mode.

    mode selects the intervention and score: "induce" adds `vector` at `layer`
    and returns refusal log-odds; "bypass" projects the direction out of every
    layer and returns refusal log-odds; "kl" projects it out and returns the KL
    divergence from clean_probs. Lower bypass and KL are better; higher induce
    is better.
    """
    scores = []
...
def main():
    """Scan every (layer, position) direction and save the best refusal direction.

    Scores each candidate on bypass, induce, and KL against the validation set,
    resuming from SCORES_FILE if present, and writes the winning direction to
    RESULT_FILE.
    """
    ...
    def consider(layer, pos, b_s, i_s, k_s):
        """Update the strict and relaxed best (bypass, config) if this candidate wins."""
        nonlocal best_bypass, best_config, relaxed_best_bypass, relaxed_best_config
    ...
    for layer in tqdm(range(NUM_LAYERS), desc="Scanning layers"):
        for pos in range(N_TOKENS_TO_SAVE):
            ...
            consider(layer, pos, b_s, i_s, k_s)
