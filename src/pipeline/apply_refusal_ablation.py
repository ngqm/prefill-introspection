...
def get_harmbench_prompts():
    """Return the standard-category HarmBench test behaviors, caching the CSV locally."""
...
def orthogonalize_matrix(matrix, direction):
    """Return matrix with its component along the unit `direction` removed."""
...
def apply_weight_orthogonalization(model, direction):
    """Orthogonalize the embedding, attention output, and MLP down weights in place.

    Projects `direction` out of every write to the residual stream so the model
    can no longer represent the refusal direction.
    """
...
def main():
    """Orthogonalize the model, save the checkpoint, and write sample generations.

    Reuses the checkpoint under SAVE_DIR if it already exists; otherwise loads
    MODEL_ID, applies the saved optimal direction, and saves the result. Then
    serves the ablated model with vLLM and dumps intervened outputs to
    OUTPUT_FILE.
    """
