...
def pooled_asr(safe, datasets=DATASETS):
    """Return the pooled control-response ASR across the given datasets.

    Pools the natural (control) responses over datasets and returns
    (pct_unsafe, n), or None if any required dataset file is missing guard
    labels.
    """
    n = u = 0
    for ds in datasets:
...
b2 = ax.bar(x + w/2, ablated,  w, label="Refusal-ablated",
            color=PALETTE["terracotta"], edgecolor="none", linewidth=0)
