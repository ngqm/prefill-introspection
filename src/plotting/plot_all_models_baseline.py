...
RQ = Path("rq1_runs")
OUT_DIR = Path("figures/ablation_comparison")
OUT_DIR.mkdir(parents=True, exist_ok=True)
DATASETS = ["HarmBench", "SocialHarmBench", "JailbreakBench", "StrongREJECT"]


def _read(safe, ds, adv):
    """Return the canonical f1 rows for one model and dataset, or None if the
    file is absent. adv selects the adversarial-prefill file."""
...
def paired(safe):
    """Return the behavior-change subset pooled across DATASETS.

    Keeps prompts whose guard label differs between the control and adv_prefill
    conditions, as (prompt_id, control_claim, prefill_claim) triples.
    """
    triples = []
    for ds in DATASETS:
...
def rates(triples):
    """Return control/prefill claim rates and their gap for the triples.

    Returns a dict with ctrl, pre, gap, and n, or None when triples is empty.
    """
    if not triples:
        return None
    n = len(triples)
...
for i, (label, safe, fam, r) in enumerate(results):
    ...
    # Annotate each marker with its claim rate value: smaller value to the left
    # of its marker, larger value to the right.
    left_val, right_val = sorted([r["pre"], r["ctrl"]])
    ax.text(left_val - 4, y[i], f"{left_val:.1f}", va="center", ha="right", fontsize=8, color="black")
    ax.text(right_val + 4, y[i], f"{right_val:.1f}", va="center", ha="left", fontsize=8, color="black")
