...
def _read(safe, ds, ftag, adv):
    """Return the canonical rows for one model, dataset, and probe (ftag), or
    None if the file is absent. adv selects the adversarial-prefill file."""
...
def paired(safe, ftag="f1", change_set=None):
    """Return the behavior-change subset on baseline pooled across DATASETS.

    Returns (triples, chg_lookup). If change_set (dict ds -> set of prompt_ids)
    is given, restrict to that prompt set (used to evaluate the ablated
    checkpoint on the same prompts that flipped at baseline).
    """
    triples = []
    chg_lookup = {}
    for ds in DATASETS:
...
def gap_ci(triples, n_boot=1000):
    """Return (point estimate, 95% lower CI, 95% upper CI, n) using the
