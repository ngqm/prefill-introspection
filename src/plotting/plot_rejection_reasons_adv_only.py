...
# Colorblind-safe (Paul Tol 'muted'). Safety-cited pair = wine/rose, intent
# pair = indigo/cyan, error pair = teal/green; the rest span sand/purple/grey.
# Decoding is also backed by the fixed category order and in-segment numbers.
COLORS = {
    "ethical_policy_violation":     "#882255",  # wine
    "safety_protocol_failure":      "#CC6677",  # rose
    "lack_of_intent":               "#332288",  # indigo
    "hypothetical_framing":         "#88CCEE",  # cyan
    "poor_phrasing":                "#44AA99",  # teal
    "factual_inaccuracy":           "#117733",  # green
    "training_data_artifact":       "#DDCC77",  # sand
    "misinterpretation_of_request": "#AA4499",  # purple
    "other":                        "#BBBBBB",  # grey
}
LABEL = {
...
# Light fills take dark text; the darker fills (wine, rose, indigo, green,
# purple) take white text.
LIGHT_FILL = {"hypothetical_framing", "poor_phrasing",
...
def _clean_label(model_safe):
    """Return the display name for a safe model slug."""
    return DISPLAY[model_safe]


def main():
    """Build the per-model stacked bar chart and write it to OUT.

    Reads CLASSIFICATIONS, restricts to MODELS_KEEP and the canonical prompt
    set, tallies each model's rejection categories, and saves the figure.
    """
