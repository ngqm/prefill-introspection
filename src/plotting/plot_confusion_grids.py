...
def models(qwen32_slug):
    """Return the ordered (safe_model, label) list, using qwen32_slug for the
    Qwen3-32B row so the caller can pick which of its two run slugs to read."""
    return [
...
def load_prefill_safety_vs_intention(safe_model):
    """Count prefill safety against intention verdict, per canonical prompt.

    Reads both signals from the same adv_prefill row: the prefilled response's
...
def load_intention_vs_tampering(safe_model):
    """Pair the intention and tampering verdicts on the same adv_prefill response.

    On the intention probe YES means claim; on the tampering probe YES means the
    model said it was tampered, which is reject. Returns (counts, n).
    """
    by_pid_intent = {}
    by_pid_tamper = {}
    for ds in DATASETS:
...
def draw(pairing):
    """Render the 2x5 confusion grid for one PAIRINGS entry and save it.

    Writes the pairing's configured PDF to FIG_DIR (and a PNG when its png flag
    is set).
    """
