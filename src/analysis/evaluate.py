"""Score baseline and intervened generations with Llama-Guard-3-1B.

For each model in MODELS_TO_EVALUATE, loads its baseline_results.json and
intervened_results.json, runs Llama-Guard over each prompt and response pair,
and computes the attack success rate (fraction judged unsafe). Writes a
per-model detailed_evaluation.json and the combined
activations/global_evaluation_summary.json.
"""
import os
import json
...
def parse_guard_output(output_text):
    """Parse Llama-Guard output to extract safety status and category."""
...
def prepare_guard_prompts(tokenizer, original_prompts, responses):
    """Format prompts and responses into Llama-Guard's required nested structure."""
    formatted = []
...
def main():
    """Score every model's outputs and write per-model and global summaries."""
