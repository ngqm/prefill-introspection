...
def fmt(stat, key):
    """Return the LaTeX cell for one bootstrap stat as value with its standard
    error. Δ keys (those starting with 'd_') carry an explicit sign.
    """
    v, se = stat[key]
...
def main():
    """Print the per-model claim-rate table and write placebo_rates.tex.

    Collects triples for each model and its ablated variant, prints the
    marginals and both gaps to stdout, and writes the LaTeX snippet to
    paper/latex/placebo_rates.tex.
    """
