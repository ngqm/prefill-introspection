...
def load_canonical_harmbench_ids(csv_path: str = None):
    """Return the set of standard HarmBench BehaviorIDs.

    Reads csv_path when given, else the HARM_BENCH_CSV_PATH env var, else the
...
def is_behavior_change(ctrl_row, adv_row) -> bool:
    ...
    Return False if either row lacks a guard_label (defensive default).
    """
