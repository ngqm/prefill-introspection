...
def _collect_claims(path: Path, condition: str, ds: str, target: dict):
    """Index 0/1 claim labels of one condition by (ds, prompt_id) into target.
    ...
    for line in path.open():
        ...
        if (r.get("condition") == condition
            ...
            target[(ds, r["prompt_id"])] = int(r["classifier_label"] == "YES")


def load(safe: str, tag: str):
    """Return (control, adv) 0/1 claim arrays over the shared canonical prompts
    for the run tagged tag.
    """
    ctrl = {}
    adv = {}
    for ds in DATASETS:
        _collect_claims(RQ / f"classified_gen_{safe}{tag}_{ds}_f1.jsonl",
                        "control", ds, ctrl)
        _collect_claims(RQ / f"classified_gen_adv_{safe}{tag}_{ds}_f1.jsonl",
                        "adv_prefill", ds, adv)
...
def se(c, a):
    """Return the bootstrap SE (in %) of the gap mean(c) - mean(a)."""
...
def mean_se(rand_results):
    """Joint bootstrap SE for the mean gap across seeds.

    Requires all seeds indexed by the same prompt set (keys union over loaded
    keys). Resamples indices once per bootstrap draw and averages the resulting
    per-seed gaps.
    """
    rng = np.random.default_rng(seed=42)
...
def main():
    """Print base, refusal-ablated, and averaged random-direction gaps per model."""
