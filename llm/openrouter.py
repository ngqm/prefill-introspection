"""OpenAI-compatible client for hosted base models via OpenRouter.

OpenRouterClient disables provider-side reasoning, applies the Qwen `/no_think`
soft switch to user turns, and retries chat completions with exponential backoff.
Serves base models only (no local ablated checkpoints).
"""
import time
import random
import json
...
class OpenRouterClient(BaseLLMClient):
    ...
    Builds an OpenAI client against the OpenRouter base URL and sets extra_body
    to disable provider-side reasoning and parameter requirements.
    """
    ...
    def generate_chat(self, messages: List[Dict[str, str]], max_tokens: int, **kwargs) -> Tuple[str, Any]:
        """Send a chat completion and return (text, raw response) with retries."""
        processed_messages = []
        for msg in messages:
            if msg["role"] == "user":
