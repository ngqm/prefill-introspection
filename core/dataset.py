"""Load and filter the harmful-prompt benchmark datasets.

DatasetLoader dispatches on a dataset name to return a normalized DataFrame
(columns prompt_id, category, sub_topic, type, prompt_text) for
SocialHarmBench, HarmBench, JailbreakBench, or StrongREJECT. The JBB and
StrongREJECT loaders apply a token-Jaccard near-duplicate filter against
HarmBench-standard / AdvBench (and, for StrongREJECT, JBB and SocialHarmBench)
and cache the surviving subset to a frozen CSV under data/.
"""
import re
import pandas as pd
...
class DatasetLoader:
    """Load a benchmark dataset by name into a normalized DataFrame.

    The HarmBench CSV path (from config) doubles as the overlap-filter reference
    for JailbreakBench and StrongREJECT de-duplication.
    """
    ...
    def load(self, dataset_name: str) -> pd.DataFrame:
        """Load and normalize the dataset named by dataset_name.

        Dispatch on a case-insensitive name to one of socialharmbench,
        harmbench, jailbreakbench, or strongreject. Raise ValueError for an
        unknown name.
        """
    ...
    def _load_socialharmbench(self) -> pd.DataFrame:
        """Load SocialHarmBench, preferring the frozen local CSV when present.

        Fall back to the psyonp/SocialHarmBench HF Hub split, ensuring the
        expected metadata columns exist and ordering prompt_text after them.
        """
    ...
    def _load_harmbench(self) -> pd.DataFrame:
        """Load HarmBench from the local CSV, keeping only standard behaviors.

        Raise FileNotFoundError if the CSV is missing.
        """
    ...
    def _load_jailbreakbench(self) -> pd.DataFrame:
        """Load the JBB-Behaviors harmful split, filtered against overlaps.

        Keep only Source==Original rows, then drop any that token-Jaccard
        near-match HarmBench-standard or AdvBench. The filter runs once and
        caches its output to data/jailbreakbench_filtered.csv so the pipeline
        always sees the same prompt set across stages.
        """
    ...
    def _load_strongreject(self) -> pd.DataFrame:
        """Load StrongREJECT (Souly et al., 2024), filtered against overlaps.

        Drop rows StrongREJECT itself attributes to AdvBench, then run the same
        token-Jaccard filter against HarmBench-standard / AdvBench /
        JBB-Originals / SocialHarmBench so the surviving subset is disjoint from
        anything else in this project. Cached to
        data/strongreject_filtered.csv.
        """
...
def _norm_for_overlap(s: str) -> str:
    """Return s lowercased with non-alphanumeric characters collapsed to single spaces."""
    s = str(s).lower().strip()
    s = re.sub(r"[^a-z0-9 ]", " ", s)
    s = re.sub(r"\s+", " ", s)
    return s