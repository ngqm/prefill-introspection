...
def parse_id(rid):
    """Split a "::"-joined reply id into (model, prompt_id, cond, prefill_id, followup_id), empties as None."""
...
def load_human(probe):
    """Load the human labels for a probe, or return an empty list if the file is missing."""
...
def find_roberta_label(rid):
    ...
    prefix = "gen_adv" if cond == "adv_prefill" else "gen"
    candidates = [f"classified_{prefix}_{model}_{ds}_{suffix}.jsonl"
...
def cohen_kappa(pairs):
    """Return Cohen's κ for binary labels, where pairs is a sequence of (a, b)."""
    n = len(pairs)
    if n == 0:
        return None
...
def main():
    """Report κ and accuracy of RoBERTa and GPT-4.1 against the human labels for the --probe."""
