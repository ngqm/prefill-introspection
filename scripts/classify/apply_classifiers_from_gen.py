...
Target files are given as one or more glob patterns on the command line
(resolved against rq1_runs/); with no arguments it defaults to the gemma-3
ablated f1/f2 files. Only names containing `_f1.jsonl` or `_f2.jsonl` are
classified, and `_shard` partials are skipped. The F1 file is scored with the
F1 classifier and the F2 file with the F2 classifier.
...
Usage:
  python -m scripts.classify.apply_classifiers_from_gen [glob ...]
"""
...
def load_clf(model_dir):
    """Load the tokenizer and sequence-classification model from model_dir onto DEVICE."""
...
def predict_batch(tok, mdl, texts):
    """Return a YES/NO label (argmax over logits) for each text in the batch."""
...
def process_gen_file(gen_path: Path):
    """Classify one gen f1/f2 file and write classified_<name> to rq1_runs.

    Return the output path, or None when the file is neither an f1 nor an f2
    file. If the output already exists it is left in place and returned.
    """
    fname = gen_path.name
...
def main():
    """Classify every gen file matched by the CLI globs (default: gemma-3 ablated)."""
