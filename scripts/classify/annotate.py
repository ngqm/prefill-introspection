...
def load_jsonl(p):
    """Load rows from a JSONL file, or return an empty list if it does not exist."""
    if not p.exists():
        return []
...
def wrap(s):
    """Word-wrap a string to the display width, returning "(empty)" for falsy input."""
    if not s:
        return "(empty)"
...
def show(r, idx, total, probe, labeled_so_far, by_label):
    """Render one reply and the probe's labeling instructions on a cleared screen."""
    clear()
...
def main():
    """Run the resumable annotation loop for the --probe (F1 or F2) sample.

    Reads the sample and existing labels, walks the annotator through each
    unlabeled reply collecting YES/NO/SKIP, and appends labels to
    rq1_runs/human_labels_<probe>.jsonl. Supports back-step and quit-and-save.
    """
