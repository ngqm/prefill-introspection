"""Add a `guard_label` (safe|unsafe) field, plus optional `guard_category`, to gen_*.jsonl rows.

Run Llama-Guard-3-1B on each (prompt_text, initial_response) pair and write
the label back in place (atomic replace via a .tmp file). Idempotent: rows
that already carry a `guard_label` are skipped, and rows with an empty
response are left untouched.

Reads and writes files under OUT_DIR (from core.config), matching --pattern
and excluding names that start with 'judged_'.

Command-line arguments:
  --pattern        glob inside rq1_runs/ (default 'gen_*.jsonl')
  --filter         substring filter on filenames
  --limit-files    process at most this many files (0 = all)
  --guard-model    HF repo id of the guard model (default Llama-Guard-3-1B)
  --tp             vLLM tensor-parallel size
  --gpu-mem        vLLM gpu_memory_utilization
...
def parse_guard_output(text: str) -> Tuple[str, str]:
    r"""Parse a Llama-Guard verdict into (label, category).

    Llama-Guard outputs 'safe' or 'unsafe\nS<n>'. Return the lowercased label
    and the category code (empty unless unsafe), or ('unknown', <snippet>)
    when the first line is neither 'safe' nor 'unsafe'.
    """
...
def make_guard_prompt(tokenizer, user_prompt: str, assistant_response: str) -> str:
    """Render the (user, assistant) pair as a guard prompt via the chat template."""
    convo = [
...
def needs_guard(row) -> bool:
    """Return True if the row has a non-empty response but no guard_label yet."""
...
def process_file(path: Path, llm: LLM, tokenizer, sampling_params: SamplingParams) -> dict:
    """Score the un-labeled rows of one file and rewrite it in place.

    Guard-label every row that needs it, batching through the shared llm, then
    atomically replace path via a .tmp file. Return a summary dict with the
    file path, counts, and per-label category tally (or skipped=True when
...
def main():
    """Parse arguments, load the guard model, and score every matching file."""
