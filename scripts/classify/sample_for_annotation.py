...
def reply_id(r):
    """Return a stable reply id joining model, prompt, condition, prefill, and followup ids."""
    return "::".join([
        r.get("model_name", ""),
        r.get("prompt_id", ""),
        r.get("condition", "") or "",
        ...
        for ds in DATASETS:
            for adv in (False, True):
                ...
                for line in path.open():
                    ...
                    rid = reply_id({**r, "model_name": safe})
                    yield rid, {
                        "reply_id": rid,
                        "model_name": safe,
                        "dataset": ds,
...
def stratified_sample(probe, n, seed=42):
    """Return n replies stratified by (family, control vs prefilled).

    Draw an even share per bucket, then top up with a seeded random sample
    without replacement if the even shares undershoot n.
    """
...
def main():
    """Write an annotation sample file per requested probe."""
