"""Judge gen_*.jsonl followup replies with an OpenRouter model and write judged_*.jsonl.

For each generation file under OUT_DIR (from core.config), run the
IntrospectionJudge on every row's followup_response and write a parallel file
named <out-prefix><original-name> carrying a `judge` (YES/NO/ERROR) and
`judge_model` field. Files whose judged output is already complete are
skipped. When available, the victim model's tokenizer is loaded (parsed from
the filename) so the judge can truncate consistently. After judging, print an
aggregated YES-rate summary per (model_name, condition).

Command-line arguments:
  --judge-model  OpenRouter model id (default 'openai/gpt-4.1')
  --dataset      substring filter on dataset in the filename
  --model        target victim model whose generations to judge
  --out-prefix   prefix for output files (default 'judged_')
...
def process_file(file_path: Path, judge: IntrospectionJudge, out_prefix: str = "judged_") -> Path:
    """Judge every row of one generation file and write the judged output.

    Skip the work when the judged file is already complete. Judge rows
    concurrently, then overwrite <out_prefix><name> with the results. Return
    the judged file's path.
    """
...
def summarize_results(judged_files: list):
    """Print the aggregated YES/NO rate per (model_name, condition).

    Skip the per-model breakdown when the gen schema carries no model id.
    """
    all_rows = []
...
def run_judge_pipeline(judge_model: str, target_dataset: str, target_model: str, out_prefix: str = "judged_"):
    """Judge all matching generation files and print the aggregated summary.

    Select gen_*.jsonl files under OUT_DIR, narrow by the optional dataset and
    model filters, judge each with judge_model via OpenRouter, and summarize.
    """
