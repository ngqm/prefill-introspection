...
Then apply the existing v3 F1 RoBERTa classifier locally
and compute the gap.

Usage:
  python -m scripts.finetune.eval_intros_lora --model Qwen/Qwen3-8B \\
         --adapter weights/intros_lora_v1/Qwen__Qwen3-8B/final
"""
import argparse
import json
...
@torch.no_grad()
def main():
    """Generate the LoRA's F1 answers on the held-out test set via PEFT and write them."""
