...
def main():
    """Generate control and adv_prefill responses with the LoRA model, score
    them with Llama-Guard-3-1B, and write the gens, judged, and summary files.
    """
    ...
    stem = f"asr_lora_{safe}_{args.dataset}_epoch{args.epoch}{suffix}"
    gens_path = out_dir / f"{stem}_gens.jsonl"
    judged_path = out_dir / f"{stem}_judged.jsonl"
    ...
    with gens_path.open("w") as f:
        for pid, ctrl_o, adv_o in zip(prompt_ids, ctrl_out, adv_out):
            prompt = pid_to_prompt[pid]
            prefix = prefixes[pid][0]
            for condition, response in (
                ("control", ctrl_o.outputs[0].text.strip()),
                ("adv_prefill", prefix + adv_o.outputs[0].text),
            ):
                f.write(json.dumps({
                    "prompt_id": pid,
                    "condition": condition,
