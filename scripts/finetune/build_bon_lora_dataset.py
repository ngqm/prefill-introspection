...
Each row matches the v1 schema (messages: [user, assistant, user, assistant])
...
def load_rows(safe_model: str, dataset: str, kind: str):
    """Load canonical judged rows for one dataset and kind ('control' or 'adv').

    Return the rows with a non-empty initial_response, or an empty list when
...
def judge_texts(tok, mdl, texts):
    """Run the RoBERTa F1 classifier on a list of follow-up texts.

    Return a list of "YES" / "NO" predictions, one per input text.
    """
    preds = []
...
def main():
    """Sample, F1-judge, and filter candidates into a best-of-N LoRA train set."""
