...
def _load(safe, dataset, kind):
    """Load canonical judged rows for one dataset and kind ('ctrl' or 'adv').

    Return the rows with a non-empty followup_response, or an empty list when
    the source file is missing. Raise ValueError on an unknown kind.
    """
    if kind == "ctrl":
...
def is_correct(row):
    """Return True when the row's classifier_label verdict matches its ground-truth condition."""
...
def to_record(r, dataset):
    """Convert a judged row into an HF SFT record (messages + meta)."""
    return {
        "messages": [
...
def main():
    """Build the self-distillation train/test splits and length stats for one model."""
    ...
    for ds in DATASETS:
        ...
        for kind_rows, corr_key, tot_key in ((ctrl, "ctrl_correct", "ctrl_total"),
                                             (adv, "adv_correct", "adv_total")):
            for r in kind_rows:
                counts[split][tot_key] += 1
                correct = is_correct(r)
                if correct:
                    counts[split][corr_key] += 1
                if split == "train" and correct:
                    train_rows.append(to_record(r, ds))
                    lengths[corr_key].append(len(r["followup_response"]))
                elif split == "test":
