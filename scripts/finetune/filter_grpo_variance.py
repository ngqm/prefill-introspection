...
Usage:
  python -m scripts.finetune.filter_grpo_variance \\
    ...
    --out-tag lora_intros_varfilt_v1 \\
...
def main():
    """Sample base-policy rollouts, score their reward variance, and keep high-variance prompts.

    Parse CLI args, generate --n-rollouts completions per prompt with vLLM,
    score them with the f1_classifier reward, and write the prompts whose reward
    standard deviation reaches --min-std to the output train.jsonl plus a
    filter_stats.json summary.
    """
    ...
    # Frees ~70% of GPU for the classifier.
    del llm
    import gc
    gc.collect()
