...
def build_dataset(data_path: Path, tok) -> Dataset:
    """Return a dataset of prompts plus condition for reward routing.

    Read the SFT jsonl at data_path, strip the final assistant turn from each
    row's messages, and re-render the prefix with add_generation_prompt so the
    policy generates the F1 answer. condition (control vs prefilled) rides along
    for reward_fn routing.
    """
    rows = []
...
def main():
    """Train a GRPO LoRA with the F1 classifier as reward and save the final
    adapter to weights/<out_tag>/<safe>/epoch1.
    """
    ...
    def reward_fn(prompts, completions, condition, **kwargs):
        """Reward P(YES) for control prompts and P(NO) for prefilled prompts."""
