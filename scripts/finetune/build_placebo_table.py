...
def load_benign(safe):
    """Load the model's benign completions, or None if the file is absent."""
...
def build(safe, dataset):
    """Write the placebo-prefill table for one model and dataset.

    For each adv_prefill row, truncate a deterministically chosen benign
    completion to the length of that row's prefill and record it. Skip the
    model or dataset when its benign or adv_prefill source file is missing.
    """
