...
def load_rows(safe_model: str, dataset: str, kind: str):
    """Return the canonical classified F1 rows for one (model, dataset) cell.

    kind selects the source file: 'control' reads classified_gen_* and keeps
    only control-condition rows; 'adv' reads classified_gen_adv_*. Rows are
    filtered to canonical prompt ids with a non-empty initial_response. Returns
    an empty list when the source file is absent.
    """
    if kind == "control":
...
def main():
    """Sample candidates, score them with the F1 classifier, build preference
    pairs, and write data/<out_tag>/<safe>/train.jsonl plus stats.json.
    """
