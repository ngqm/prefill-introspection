...
class JudgeDataset(Dataset):
    """Torch dataset of tokenized reply texts with integer YES/NO labels.

    __init__ tokenizes texts once (truncated and padded to MAX_LEN) and stores
    labels as a long tensor; __getitem__ returns the encodings plus the label
    for one example.
    """
...
def metrics(pred):
    """Return accuracy, precision, recall, and binary f1 for a Trainer prediction."""
