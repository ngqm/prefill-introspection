...
def load_benign_prompts():
    """Return the non-empty, stripped lines of data/benign_prompts.txt."""
...
def run(model_name, tp, mem_fraction_static, port=None):
    """Generate one benign completion per prompt and write them to OUT_DIR.

    Resolve model_name to its non-ablated checkpoint, launch a vLLM server, and
    write one row per benign prompt to benign_completions_<safe_mname>.jsonl. Do
    nothing when that file already exists and is non-empty. port defaults to
    $VLLM_PORT or VLLM_DEFAULT_PORT when None; tp and mem_fraction_static are
    passed through to the vLLM server.
    """
    set_seed()
