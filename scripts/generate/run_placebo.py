...
def load_placebo_table(safe_base, dataset):
    """Read the base-model table even for the ablated checkpoint.

    Keeps base and ablated placebos using identical prefill text.
    """
...
def process_row(client, row, placebo_map):
    """Run one prompt's placebo trial and return its output row.

    Look up the prompt's placebo prefill in placebo_map and run trial();
    return an error row (condition='error') when the prompt has no placebo or
    the trial raises.
    """
    pid = row["prompt_id"]
...
def _load_done_prompt_ids(path):
    ...
    for line in path.open():
        try:
            ...
            # Skip a truncated final line or a row without prompt_id.
            pass
    return done


def run(model_name, dataset_name, ablated, tp, mem_fraction_static, concurrency=64, port=None):
    """Run the placebo condition for (model, dataset) and stream per-probe rows.

    Load the base-model placebo table, launch vLLM, run each prompt's placebo
    prefill plus the F1-F4 followups concurrently, and append results to the
    gen_placebo_<safe>_<dataset>_f{1,2,3,4}.jsonl files. Already-written prompt
    ids are skipped so a partial run resumes.
    """
    set_seed()
